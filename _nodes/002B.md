...
{%ref 0008 %} if and only if the following two conditions hold:
...
$E$ has hypocartesian lifts because it has cartesian lifts.

TODO: closure under composition
...
«
  ...
  \end{tikzpicture}
»
Let $\bar{m}:\bar{u}\Sup{\prime}\to\Sub{m}\bar{x}$ be the hypocartesian lift of
$m$ at $\bar{x}$, where $\bar{u}\Sup{\prime}\in E\Sub{u}$. By hypothesis, the
composite $\bar{m};\bar{f} : \bar{u}\Sup{\prime}\to\Sub{m;f}\bar{y}$ is
...
factorization of $\bar{h}$ through $\bar{f}$ over $m$.
